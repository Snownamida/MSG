...
def _structural_sig(R: Rom, n: int):
    ...
    while i < len(raw):
        ...
        if b in TRIPLE_BYTE: ctrls.append(raw[i:i+3].hex()); i += 3
            ...
            if all(c == 0 for c in R.block_ppu(b)): setups.append(b)
            i += 1
        else: i += 2
    return ctrls, folds, setups
...
if __name__ == "__main__":
    ...
    for n in range(1, 2782):
        ...
        c1, f1, st1 = _structural_sig(R, n)
        c2, f2, st2 = _sig_from_structured(s)
        # 控制码：DOUBLE 在 sig 里步进有 bug，改用宽松比对（数量+集合）
        if f1 != f2 or sorted(st1) != sorted(st2):
            bad += 1
            if bad <= 3:
                print(f"句{n} 不一致: 折行 {f1}vs{f2}, 设置 {sorted(st1)}vs{sorted(st2)}")
    print(f"折行/设置块 round-trip: {2781 - bad}/2781 一致" + (" ✓" if not bad else ""))
    print()
