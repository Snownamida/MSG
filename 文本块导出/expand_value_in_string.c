...
struct string block_string_to_string_ppu(struct string block_string) {
	...
	for (int block_count = 0; block_count < block_string.length; block_count++) {
		...
		if (block_string.string[block_count] < 0x80) {
			if (block_string.string[block_count] == 0x06 && block_string.string[block_count + 1] != 0x00)printf("这句话原来可能不全（06）：\n");
			if (block_string.string[block_count] == 0x04 && block_string.string[block_count + 1] != 0x00)printf("这句话原来可能也不全（04）：\n");
